...
const App: React.FC = () => {
  ...
  const handleGarmentSelect = useCallback(async (garmentFile: File, garmentInfo: WardrobeItem, color?: string) => {
    if (!displayImageUrl || isLoading) return;

    // Caching: Check if we are re-applying a previously generated layer with the same color
    const nextLayer = outfitHistory[currentOutfitIndex + 1];
    if (nextLayer && nextLayer.garment?.id === garmentInfo.id && nextLayer.color === color) {
    ...
    setError(null);
    setIsLoading(true);
    setLoadingMessage(`Adding ${garmentInfo.name}${color && color !== 'Original' ? ` (${color})` : ''}...`);

    try {
      const newImageUrl = await generateVirtualTryOnImage(displayImageUrl, garmentFile, color);
  ...
  const handleSaveLook = (name: string) => {
    if (!displayImageUrl) return;
    
    const garmentColors = activeOutfitLayers
      .map(layer => layer.color)
      .filter((c): c is string => !!c && c !== 'Original');
  ...
  const handleSelectLook = (look: SavedLook) => {
    // Treat the saved look as a new base model
    handleModelFinalized(look.url);
  };
